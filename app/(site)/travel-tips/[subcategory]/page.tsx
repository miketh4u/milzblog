...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `${label} — Travel Tips` };
}

export default async function TravelTipsSubcategoryPage({ params }: Props) {
  const { subcategory } = await params;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
      <nav className="flex items-center gap-2 text-[var(--color-text-muted)] text-sm mb-8">
        <Link href="/travel-tips" className="hover:text-[var(--color-primary)] transition-colors">Travel Tips</Link>
        <span>/</span>
      ...
      {posts.length > 0 ? (
        ...
        <div className="text-center py-24 text-[var(--color-text-muted)]"><p>No posts in this category yet.</p></div>
      )}
    </div>
  );
}
