...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `${label} — Travel News` };
}

export default async function TravelNewsSubcategoryPage({ params }: Props) {
  const { subcategory } = await params;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
      <nav className="flex items-center gap-2 text-[var(--color-text-muted)] text-sm mb-8">
        <Link href="/travel-news" className="hover:text-[var(--color-primary)] transition-colors">Travel News</Link>
        <span>/</span>
